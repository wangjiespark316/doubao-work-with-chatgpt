...
program
  .name("c2c")
  .description(`${PRODUCT_NAME} — ChatGPT thinks. Doubao Work works.`)
...
program
  ...
  .action(async (opts: { workspace?: string; tunnel: boolean; json: boolean }) => {
    ...
    try {
      ...
      say("如果你在使用 Doubao Work Skill，这一步会自动完成。");
    } catch (error) {
      handleCliError(error, opts.json);
    }
  });
...
program
  ...
  .action(async (opts: { workspace?: string; fix: boolean; json: boolean }) => {
    ...
    // State directory writability (Doubao Work has no sandbox; this is a no-op check)
    if (opts.fix) {
      const sandbox = trySandboxAllow();
      if (sandbox.ok) {
        report.sandbox = { ok: true, detail: "本地目录可写" };
        if (sandbox.added) results.push("本地设置目录已就绪");
      } else {
      ...
      try {
        ...
        const writable = fs.existsSync(stateDir);
        report.sandbox = writable ? { ok: true, detail: "本地目录可写" } : { ok: false, detail: "本地目录不存在" };
      } catch (error) {
...
// ---------------------------------------------------------------- sandbox-allow (no-op on Doubao Work; kept for Skill compatibility)

acceptUnusedWorkspaceOption(
  program
    .command("sandbox-allow")
    .description("Ensure the local settings directory is writable (no-op on Doubao Work)")
    .option("--json", "machine-readable output", false)
)
  .action((opts: { json: boolean }) => {
    ...
    if (!result.ok) {
      cross(`本地设置目录不可写：${result.error}`);
      process.exitCode = 1;
      return;
    }
    if (result.alreadyAllowed) check("本地设置目录已就绪，后续对话无需再提权");
    else check("本地设置目录已创建（后续对话无需再提权）");
  });
