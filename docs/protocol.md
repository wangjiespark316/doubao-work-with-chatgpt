...
| --- | --- | --- |
| INIT | Doubao Work | New task; asks ChatGPT to inspect + plan |
| PLAN | ChatGPT | Executable plan for the next iteration |
| EXECUTING | Doubao Work | (optional) execution in progress |
| EXECUTED | Doubao Work | Iteration finished; metadata only |
...
| HANDOFF | Doubao Work | Continuation brief sent to a replacement conversation |

There is no `STATE: RESUME`. If Doubao Work restarts mid-task, it reads a **local
...
only the table above. If the original chat is gone, Doubao Work sends HANDOFF
...
| `EXECUTING` | Doubao Work is applying the current PLAN |
...
INSTRUCTION:
Inspect the connected workspace through Doubao Work with ChatGPT MCP.
Create an implementation plan for Doubao Work.
```

### PLAN (ChatGPT → Doubao Work)
...
Before sending EXECUTED, Doubao Work records the iteration:
...
is a separate opt-in: `execution_output` (`list` then `read`). Doubao Work nominates
...
### DONE / BLOCKED (ChatGPT → Doubao Work)
...
### HANDOFF (Doubao Work → new ChatGPT conversation)

`c2c session --json` → `conversation.mode` chooses how chats are grouped.

- **long-chat:** one long-lived C2C conversation per workspace. Doubao Work opens a
  replacement chat only when the user asks, the old chat lags, or the chat was
  lost.
- **project:** one ChatGPT Project (collection) per workspace. A new Doubao Work
  conversation starts a new chat **inside that Project**. The same Doubao Work
  conversation keeps using its saved chat URL.

Right after the boot prompt, Doubao Work sends a HANDOFF so the new chat can
...
`maxIterations` (default 12, configurable in `.c2c.json`). When reached, Doubao Work
...
```
You are the planning and review layer of a Doubao Work coding session.

Doubao Work owns execution.
You own high-level reasoning, planning and review.

You have access to the current local workspace through the
"Doubao Work with ChatGPT" MCP connector.

Rules:

1. Do not ask Doubao Work to paste files that are available through MCP.
...
5. Doubao Work will execute your plan using its own harness.
6. After Doubao Work reports EXECUTED, independently inspect the diff.
   ...
   from git.
7. Do not assume an implementation succeeded just because Doubao Work says so.
...
11. Be substantive. PLAN and review replies must carry enough signal for
    Doubao Work to act on: rationale, per-file natural-language suggestions
