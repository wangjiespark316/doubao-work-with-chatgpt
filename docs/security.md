...
| Execution output leak | Doubao Work may nominate test/build/lint logs; a local sanitizer redacts tokens, pairing-code-shaped strings and home paths, truncates size, and refuses private-key blocks entirely. Restricted items are listed without a body. ChatGPT still cannot run commands. |
...
(`~/Library/Application Support/doubao-work-with-chatgpt` on macOS), directories 0700,
