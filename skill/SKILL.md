...
description: >
  ...
  "使用 Doubao Work with ChatGPT ..." / "Set up Doubao Work with ChatGPT" /
  "用 ChatGPT 规划", when they ask to connect ChatGPT to the current workspace,
  disconnect it, or run a task through the ChatGPT planning loop.
---

# Doubao Work with ChatGPT

ChatGPT thinks. Doubao Work works.

You (Doubao Work) own execution: editing, shell, git, tests, recovery.
...
4. If something fails, run `c2c doctor` and repair silently. Only involve the user
   ...
   once.
5. ALWAYS use the built-in browser for every ChatGPT step. Load the
   `browser-use-automation-mac` skill and use `mac_computer_use_tool` with
   `plane="bu"` (the `bu` library) to control the page. NEVER use desktop
   Computer Use (`cu`) for clicking inside a web page. NEVER launch or
   control a third-party/external browser (Chrome, Safari, Edge…), and never
   use `open <url>` to hand off to one.
   ...
   - If the user asks to run ChatGPT in their own browser, refuse politely and
     explain: "我需要持续调用 ChatGPT 和配置连接，这会频繁操作页面，可能影响
...
6. Conversation reuse depends on `c2c session --json` → `conversation.mode`
   ...
   - **project** (new workspaces, or an existing workspace that opted in):
     ONE ChatGPT Project (collection) per workspace. The same Doubao Work
     conversation reuses the ChatGPT chat URL saved in THIS thread. A new
     Doubao Work conversation opens a new chat from the Project collection page
     — never navigate to `https://chatgpt.com/` to create it, and never reuse
     another conversation's chat URL just because `session.url` exists.
...
7. After first-time setup, never ask the user to approve writing C2C's local
   settings directory. Run `c2c sandbox-allow --json` (idempotent; on Doubao
   Work this is a no-op that ensures the state directory exists). If it fails
...
8. ChatGPT pages: only the URLs in **Browser automation (ChatGPT)**. Never start
   from chatgpt.com and click through menus.
9. **Doctor gate.** After `c2c doctor --json`, do not open ChatGPT and do not
...
Use the `browser-use-automation-mac` skill. Drive the page with
`mac_computer_use_tool` and `plane="bu"` (the `bu` library). These C2C rules
override defaults that close the tab, hide the window, or stall on the settings page.

1. **Surface.** Reuse one browser tab for all ChatGPT steps. If a tab with
   chatgpt.com is already open in the built-in browser, reuse it; otherwise
   open a new tab. Never open a second ChatGPT tab. Do not navigate to the URL
   you are already on.

2. **Foreground + keep.** Right after opening or claiming the tab, bring the
   browser window to the foreground so the user can watch. Never close this tab.
   Finished, waiting for the user, or timed out: leave it open. Do not let
   default turn cleanup close it.

3. **URLs only** (same tab, navigate directly — never hunt menus):
   ...
   - Saved C2C chat: `conversation.chatUrl` / `session.url` (long-chat, or
     the chat already bound in THIS conversation)
...
5. **Batch.** Fill a known form in one script when you can. After an action,
   one cheap DOM check via `bu.snapshot()`. Do not screenshot-poll.
...
6. **One conversation, Chat mode.** The first ChatGPT chat is the C2C
   ...
   conversation. If no switcher is visible, do not hunt menus; continue.
   ...
   chat passes workspace_info. Do not `session clear`.

7. **Wait for a ChatGPT reply (do not hold one long browser wait).** After you
   send INIT, EXECUTED, boot, or the workspace_info check: keep the tab
   foreground, and stay in this same task. Do not wait 5 minutes in one call
   and do not screenshot-poll. Every 20–30 seconds, one cheap `bu.snapshot()`:
   ...
   A browser timeout is not failure. Claim the same tab, read the page, keep
   it open. If ChatGPT is still thinking, keep polling. Never open a second
...
2. `c2c sandbox-allow --json` (do not pass `-w`) — ensures the C2C state
   directory exists (macOS: `~/Library/Application Support/doubao-work-with-chatgpt`;
   Windows: `%LOCALAPPDATA%\doubao-work-with-chatgpt`). On Doubao Work this is
   a no-op; if already present, it does not trigger elevation.

- `{ "updateAvailable": false }` → continue silently. Never mention the check.
- `{ "updateAvailable": true }` → tell the user one line:
  "检测到 Doubao Work with ChatGPT 有新版本，我先更新一下（约 1 分钟），随后继续你的任务。"
  Then run the update workflow below, and CONTINUE the original task afterwards.

## Workflow: update（"更新 Doubao Work with ChatGPT"，or triggered by the daily check）
...
3. Re-install the Skill: copy `skill/SKILL.md` to the user's Doubao Work skills
   directory (see Locations), then fix the "checkout lives at:" line in the copy
   to the actual checkout path.
4. `c2c sandbox-allow --json`, then `c2c restart -w <workspace>` so the bridge
   runs the new code, then `c2c update-check --force --json` to refresh the cache
   (should now report up to date).
5. Tell the user "✓ 已更新到最新版本" — then resume whatever task triggered this.
   (The updated SKILL.md takes effect from the next Doubao Work session; that's expected.)
...
3. Run `c2c sandbox-allow --json`, then **Connection choice**, then
   ...
   `Doubao Work with ChatGPT`; additional workspaces get `Doubao Work with ChatGPT · <name>`).
...
5. Open ChatGPT on the ONE browser tab (see **Browser automation**). Foreground
   immediately. Same tab, navigate directly:
   ...
   - 还没有 / 刚删掉: `https://chatgpt.com/plugins#settings/Connectors?create-connector=true&redirectAfter=%2Fplugins`
     Operate ONLY on `connectorName` from step 3:
      ...
      - Description: `Securely connect ChatGPT to the current Doubao Work workspace for planning and review.`
...
6. Same tab: open the first C2C chat per **Conversation management**
   ...
   in long-chat). Confirm Chat mode per **Browser automation** §6 (if it is Work,
   ...
   Confirm the reply matches `workspaceName` (wait per **Browser automation** §7).
...
path for a browser timeout without a visible error, a page that is
...
silently fall back to Doubao-Work-only execution and do not permanently disable C2C.
...
- **Find it**: if `conversation.reuseSavedChat` and `conversation.chatUrl`,
  navigate to that URL (foreground) and continue there.
...
- **Switch it** ONLY when (a) the user asks for a new chat, (b) the current
  ...
  1. Same tab: navigate to `https://chatgpt.com/`, confirm Chat mode
     (**Browser automation** §6), then send the boot prompt.
...
1. Same Doubao Work conversation (this thread still has context) → same ChatGPT
   chat URL. Navigate to that URL directly. Do not open the collection first.
2. Same workspace, a **new** Doubao Work conversation → new ChatGPT chat from the
   collection page (`conversation.projectUrl`). Ignore `session.url` unless
   you already saved it earlier in THIS thread.
3. Different workspace → different Project and different connector.

**Open a chat in this thread**

- If you already saved a ChatGPT chat URL earlier in THIS conversation:
...
- Else if `conversation.projectReady`: navigate to `conversation.projectUrl`.
  ...
  Confirm Chat mode (**Browser automation** §6). Boot prompt, then workspace_info
  ...
  If this thread is continuing a previous C2C task, send HANDOFF right
...
(desktop Computer Use is forbidden; browser automation must not hunt that menu).
...
```
You are the planning and review layer for one local workspace. Doubao Work executes.
...
Doubao Work with ChatGPT connector. If workspace_info names a different
...
## Workflow: coding task（"使用 Doubao Work with ChatGPT 完成 XXX"）
...
All control messages start with `[C2C]`. Keep Doubao Work→ChatGPT messages under 1 KB.
...
1. `c2c session -w <workspace> --json`. Open ChatGPT on the same browser tab
   per **Conversation management** for `conversation.mode` (foreground).
   long-chat: saved chat, or `https://chatgpt.com/` if none.
   ...
   confirm Chat mode (**Browser automation** §6), then send the boot prompt from
   ...
   **Resume from `session.checkpoint` before any INIT.** Missing checkpoint
   (legacy session): continue as a normal new/continued loop. A browser
...
INSTRUCTION:
Inspect the connected workspace through the Doubao Work with ChatGPT MCP connector.
Produce a C2C PLAN message.
```

   Confirm the INIT message is visibly in that ChatGPT conversation (one cheap
   DOM check). If the page is Retry-only, recover per **Browser automation** §6
...
3. Wait for ChatGPT's `STATE: PLAN` reply (**Browser automation** §7 — short DOM
...
4. Execute the plan yourself with your own tools (your tools, your judgment;
...
2. Optionally remove the connector on the same browser tab via
   `https://chatgpt.com/plugins` (foreground). Only touch
   this workspace's `connectorName`.
3. Tell the user: "已断开 ChatGPT 对该项目的访问。"

## Workflow: reconnect after address reclaim（全关掉以后地址失效）

This is the normal case when the user quit Doubao Work / the terminal / the machine:
...
2. Same one browser tab as setup (foreground). Settings URLs only
...
4. `c2c doctor --json` again. Same tab: only after the Doctor gate is green,
   reopen the chat this thread was already using (`session.url` /
...
| Same explicit ChatGPT setup/reconnect browser configuration step fails twice after repair | Stop automating ChatGPT settings and use **Guided manual ChatGPT setup fallback**. Do not count browser timeout, loading/generating, or login/2FA waiting as failures. |
